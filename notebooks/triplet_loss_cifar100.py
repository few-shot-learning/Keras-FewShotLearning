...
train_dataset, val_dataset, test_dataset = tfds.load(
    name="cifar100", split=["train[:90%]", "train[90%:]", "test"], as_supervised=True
)
...
output_dir = Path("logs") / "triplet_loss_cifar100_normalized_embeddings"
...
classifier = Sequential([encoder, Dense(100, activation="softmax")])
classifier.compile(
...
results += [
    {
        ...
        **dict(
            zip(
                ...
                classifier.evaluate(test_dataset.map(lambda x, y: (preprocessing(x), y)), steps=test_steps),
            )
        ),
    }
]
