    ...
    class TestBuildSupportSet:
        ...
        def test_should_raise_value_error_when_inputs_is_not_list():
            with pytest.raises(ValueError) as error:
                CentroidsSimilarity(kernel=sentinel.kernel).build_support_set(sentinel.inputs)
        ...
        def test_should_return_centroids_of_input_tensors_according_to_their_class():
            ...
            support_tensors = CentroidsSimilarity(kernel=sentinel.kernel).build_support_set([input_tensors, labels_one_hot])
            np.testing.assert_array_equal([[1, 1, 1, 1], [3.5, 3.5, 3.5, 3.5]], support_tensors.numpy())

    class TestCall:
        @staticmethod
        @patch("keras_fsl.models.layers.centroids_similarity.SupportLayer.call")
        def test_should_call_activation_on_super_call(mock_super_call):
            layer = CentroidsSimilarity(kernel=sentinel.kernel, activation=MagicMock(name="activation"))
