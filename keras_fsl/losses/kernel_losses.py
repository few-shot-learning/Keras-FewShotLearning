...
def pair_wise_loss(m=0.1):
    """
    Compute the binary crossentropy loss of each possible pair in the batch. The margin lets define a threshold against
    which the difference is not taken into account, ie. |y_true - y_pred| < m => loss = 0
    """
    def binary_crossentropy(y_true, y_pred):
        y_true = tf.matmul(y_true, y_true, transpose_b=True)
        return (K.cast(K.abs(y_true - y_pred) > m, 'float32')) * K.binary_crossentropy(y_true, y_pred)
